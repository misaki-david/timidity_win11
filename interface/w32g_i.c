...
// dir
int SeachDirRecursive = 0;	// 再帰的ディ?クト??索 

// Ini File
int IniFileAutoSave = 1;	// INI ファイ?の自動セーブ
...
// ***************************************************************************
// Start Window
// 大元のウィ?ドウの地位はMain Windowに譲り、今ではただの?期化関?
...
#ifdef SUBWINDOW_POS_IS_OLD_CLOSED_WINDOW
		// サブウイ?ドウを最大?を越えて閉じられる古いウイ?ドウに?わせる仕様は止めることにした。
		if(max>0){
...
static struct MPanelMessageData_ {
	...
	char curbuff[1024];
	int curbuffsize;
	int curmode;	// 現在?ッセージの?ード。
	int curmsec;		// 現在?ッセージの残り秒。
	char nextbuff[1024];
	int nextmode;	// 現在?ッセージの?ード。
	int nextmsec;		// 現在?ッセージの残り秒。
} MPanelMessageData;
...
// PanelInfo 構造体を読み?んでパネ?構造体へ適用する。
// flag は強制更新する。
void MPanelReadPanelInfo(int flag)
{
...
// sec 秒で message を流す。
// mode 0: sec 秒だけ message を表示。デフォ?ト。
// mode 1: sec 秒の間に message を右から左に流す。
// mode 2: sec 秒の間に messege を表示。ポイ?タを左から右に移す。ポイ?タを境界に色を変える。
void MPanelMessageAdd(char *message, int msec, int mode)
{
...
static void DlgDirOpen(HWND hwnd)
{
	...
	if ( PlayerLanguage == LANGUAGE_JAPANESE ) 
		bi.lpszTitle = "MIDI ファイ?の?るディ?クト?を御選択なされますよう。";
	else
