...
static int tracer_pan_draw ( RECT *lprc, int vol, int max, int lockflag )
{
	...
	// ※以下の2つの転???では無駄な?分も転?しているので、うまくやれば省ける。
	// マスクを描画
...
static int string_view_border_draw ( RECT *lprc, COLORREF back, int lockflag)
{
	...
	hPen1 = CreatePen(PS_SOLID, 1, RGB((back & 0xff) + 32, ((back & 0xff00) >> 8) + 32, ((back & 0xff0000) >> 16) + 32));
	hPen2 = CreatePen(PS_SOLID, 1, RGB((back & 0xff) - 16, ((back & 0xff00) >> 8) - 16, ((back & 0xff0000) >> 16) - 16));
	hPen3 = CreatePen(PS_SOLID, 1, RGB((back & 0xff) + 16, ((back & 0xff00) >> 8) + 16, ((back & 0xff0000) >> 16) + 16));
	hPen4 = CreatePen(PS_SOLID, 1, RGB((back & 0xff) - 32, ((back & 0xff00) >> 8) - 32, ((back & 0xff0000) >> 16) - 32));
	if ( lockflag ) TRACER_LOCK ();
	hOldPen= (HPEN)SelectObject(hdc,GetStockObject(NULL_PEN));
...
//				 "%x", wParam );
			...
			case 0x68:	// h
				...
				MessageBox(hTracerWnd,
					"キーコマ?ド\n"
					"ト?ーサウイ?ドウコマ?ド\n"
					"  ESC: ヘ?プを閉じる      H: ヘ?プを出す\n"
					"  +: キーアップ    -: キーダウ?\n"
					"  >: スピードアップ    <: スピードダウ?\n"
					"プ?イ?ーコマ?ド\n"
					"  SPACE/ENTER: ?奏開始    E: 停止    S: 一?停止\n"
					"  P: 前の曲    N: ?の曲\n"
					"TiMidity コマ?ド\n"
					"  Q: 終了\n"
					,"ヘ?プ", MB_OK);
